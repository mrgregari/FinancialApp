        ...
        Column(
            ...
            when (uiState) {
                is EditExpenseUiState.Error -> {
                    val errorResId = (uiState as EditExpenseUiState.Error).errorResId
                    ErrorScreen(
                        error = stringResource(errorResId),
                        onRetry = { viewModel.loadExpense(expenseId) }
                    )
                }
                is EditExpenseUiState.Loading -> LoadingScreen()
